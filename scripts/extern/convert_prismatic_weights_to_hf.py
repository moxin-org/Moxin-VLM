...
def convert_prismatic_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    if max_layer >= 0:
        ...
        elif hf_config.text_config.num_hidden_layers != actual_num_layers:
            print(f"[WARNING] Layer count mismatch: config={hf_config.text_config.num_hidden_layers}, actual={actual_num_layers}")
            print(f"[WARNING] Adjusting num_hidden_layers to match actual model: {actual_num_layers}")
            hf_config.text_config.num_hidden_layers = actual_num_layers
    ...
    if 'checkpoint_data' not in locals():
        checkpoint_data = torch.load(checkpoint_pt, map_location="cpu")
